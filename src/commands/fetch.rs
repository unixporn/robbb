...
pub async fn set_fetch(ctx: &client::Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    // TODO does this need better error handling?
    let mut info = lines
        .into_iter()
    ...
    // TODO normalize ram and other fields
...
pub async fn fetch(ctx: &client::Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    let fetch_info = match db.get_fetch(mentioned_user_id).await? {
        Some(x) => x,
        None => error_out!(UserErr::Other(
...
lazy_static! {
    ...
    static ref DISTRO_IMAGES: HashMap<&'static str, &'static str> = maplit::hashmap! {
