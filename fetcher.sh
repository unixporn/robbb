...
print() {
	fields=""
	[ "$NAME" ]        && fields="$fields distro:$NAME $ver"
	[ "$kernel" ]      && fields="$fields kernel:$kernel"
	[ "$term" ]        && fields="$fields terminal:$term"
	[ "$EDITOR" ]      && fields="$fields editor:${EDITOR##*/}"
	[ "$SHELL" ]       && fields="$fields shell:${SHELL##*/}"
	[ "$wm" ]          && fields="$fields de_wm:$wm"
	[ "$bar" ]         && fields="$fields bar:$bar"
	[ "$resolution" ]  && fields="$fields resolution:$resolution"
	[ "$displayprot" ] && fields="$fields display_protocol:$displayprot"
	[ "$theme" ]       && fields="$fields gtk3_theme:$theme"
	[ "$icons" ]       && fields="$fields gtk_icon_theme:$icons"
	[ "$cpu" ]         && fields="$fields cpu:$cpu"
	[ "$gpu" ]         && fields="$fields gpu:$gpu"
	[ "$ram" ]         && fields="$fields memory:$ram"
